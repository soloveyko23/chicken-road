      ...
      <defs>
        <clipPath id="clip0_57_11049">
          <rect width="49.5807" height="66.4078" fill="white"></rect>
					...
					</div>`),document.querySelector(".preloader");const L=document.querySelector(".preloader__counter"),u=document.querySelector(".preloader__line span");let p=0,i=0,m=0;t.setAttribute("data-fls-preloader-loading",""),t.setAttribute("data-fls-scrolllock",""),c.forEach(d=>{const s=document.createElement("img");s&&(s.onload=o,s.onerror=o,d.dataset.src?s.src=d.dataset.src:s.src=d.src)}),a(m);const f=()=>localStorage.setItem(location.href,"preloaded");document.querySelector('[data-fls-preloader="true"]')&&f()}else n();function n(){t.setAttribute("data-fls-preloader-loaded",""),t.removeAttribute("data-fls-preloader-loading"),t.removeAttribute("data-fls-scrolllock")}}document.addEventListener("DOMContentLoaded",preloader);function setVh(){document.documentElement.style.setProperty("--vh",`${window.innerHeight*.01}px`)}window.addEventListener("resize",setVh),setVh(),function(){let c=!1;const t={startTime:performance.now(),markPageLoadStart(){performance.mark("pageLoadStart")},markPageLoadEnd(){performance.mark("pageLoadEnd"),performance.measure("pageLoad","pageLoadStart","pageLoadEnd");const a=performance.getEntriesByName("pageLoad");console.log(`Page load time: ${a[0].duration.toFixed(2)}ms`)},trackResourceLoadTimes(){const o=performance.getEntriesByType("resource").filter(r=>r.responseEnd-r.startTime>200);o.length&&console.warn("Slow resources detected:",o.map(r=>r.name))}};function C(){t.markPageLoadStart();const a=document.querySelectorAll(".button-goto-form"),o=document.querySelector(".popup3");a.forEach(u=>{u.addEventListener("click",()=>{c=!0,document.documentElement.classList.toggle("show-form"),c&&o&&(popup2.classList.remove("popup_show"),document.documentElement.classList.add("popup-show"),document.documentElement.classList.add("popup-show"),o.classList.add("popup_show"),o.style.animation="popupAppear 0.5s forwards")})});const r=new GameRoulette,L=document.querySelector(".reset-button");L&&L.addEventListener("click",()=>r.resetGame()),t.markPageLoadEnd(),t.trackResourceLoadTimes()}function l(){const a=window.innerWidth<=550,o=document.querySelector(".page__counter"),r=document.querySelector(".game__roulette-counter"),L=document.querySelector(".page__game-logo"),u=document.querySelector(".game__roulette-game-logo");if(!(!o||!r||!L||!u))if(a)o.parentElement!==r&&r.appendChild(o),L.parentElement!==u&&u.appendChild(L);else{const p=document.querySelector(".page__counter-container"),i=document.querySelector(".page__logo-container");p&&o.parentElement!==p&&p.appendChild(o),i&&L.parentElement!==i&&i.appendChild(L)}}function e(a,o=60){const r=document.querySelector(a);if(!r||!r.children.length){console.warn(`No valid container found for selector: ${a}`);return}const L=document.createElement("div");Object.assign(L.style,{display:"flex",whiteSpace:"nowrap",willChange:"transform"}),Array.from(r.children).forEach(s=>{s.style.flex="0 0 auto",L.appendChild(s.cloneNode(!0)),L.appendChild(s.cloneNode(!0))}),r.innerHTML="",r.appendChild(L);const p=typeof o=="number"&&o>0?o:60;let i=0,m=performance.now(),f;function d(s){const g=(s-m)/1e3;m=s,i+=p*g;const h=L.offsetWidth/2;i>=h&&(i-=h),L.style.transform=`translateX(-${i}px)`,f=requestAnimationFrame(d)}f=requestAnimationFrame(d),r.addEventListener("mouseenter",()=>cancelAnimationFrame(f)),r.addEventListener("mouseleave",()=>{m=performance.now(),f=requestAnimationFrame(d)})}function n(){document.readyState==="loading"?document.addEventListener("DOMContentLoaded",C):requestAnimationFrame(C),requestAnimationFrame(()=>e(".page__marquee.marquee-row",60))}n(),window.addEventListener("resize",l()),document.addEventListener("DOMContentLoaded",l())}();const initialUrl=window.location.href,queryParams=getQueryParams(initialUrl);function getQueryParams(c){const t=c.split("?")[1];if(!t)return{};const C=new URLSearchParams(t),l={};for(const[e,n]of C.entries())l[e]=n;return l}function appendQueryParamsToLinks(){document.querySelectorAll("a").forEach(t=>{t.addEventListener("click",function(C){C.preventDefault();const l=new URL(t.href);for(const e in queryParams)l.searchParams.set(e,queryParams[e]);t.href=l.toString(),window.location.href=t.href})})}appendQueryParamsToLinks();
